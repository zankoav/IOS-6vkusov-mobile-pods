    ...
    func initBasketFormServerForRegisterUser(){
        if  Singleton.currentUser().getUser()?.getStatus() == STATUS.REGISTRED{
            if let session  = Singleton.currentUser().getUser()?.getProfile()?["session"] as? String {
                JsonHelperLoad.init(url: REST_URL.SF_BASKET_ITEMS.rawValue, params: ["key":REST_URL.KEY.rawValue as AnyObject as AnyObject,"session":session as AnyObject], act: self, sessionName: "add").startSession()
            }
